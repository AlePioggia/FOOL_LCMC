 ...
 * - per una dichiarazione, "null"; controlla la correttezza interna della
 * dichiarazione
 * - per un tipo: "null"; controlla che il tipo non sia incompleto
 * visitSTentry(s) ritorna, per una STentry s, il tipo contenuto al suo interno
 *
 * Una volta arricchito l'albero con le stEntry, posso procedere con il type
 * checking,
 * effettuato tramite visita dell'EAST, determinando i tipi delle espressioni
 * (TypeNode) in modo bottom-up.
 * Typechecking funzioni:
 * - il type checking delle espressioni usate come argomento ci dovrà dare, per
 * ogni espressione,
...
public class TypeCheckEASTVisitor extends BaseEASTVisitor<TypeNode, TypeException> {
	 ...
	 *
	 * Quando visito una dichiarazione, nel type checking faccio controlli interni,
	 * perché:
	 * - se è una dichiarazione di variabile, devo controllare che l'espressione con
	 * cui inizializzo la variabile
	 ...
	 *
	 * Con il try catch gestisco gli errori di tipo, per consentire che la
	 * compilazione continui.
	 */
	@Override
	...
	/**
	 * Controlliamo che tutte le dichiarazioni locali siano consistenti (durante la
	 * dichiarazione non serve verificare
	 * che i parametri siano corretti, la verifica avviene durante la chiamata).
	 *
	 * Corpo della funzione (che è una espressione), devo verificare che sia
	 * compatibile con il tipo
	...
	public TypeNode visitNode(FunNode n) throws TypeException {
		if (print)
			printNode(n, n.id);
		for (Node dec : n.declist) // visito tutte le dichiarazioni per verificare, induttivamente, che siano
									// corrette
			try {
				visit(dec);
			} catch (IncomplException e) {
	...
	/**
	 * Confronta il suo tipo ClassTypeNode in campo type, con quello del genitore,
	 * in campo superEntry,
	...
	public TypeNode visitNode(ClassNode n) throws TypeException {
		...
		if (n.superId != null) {
			...
			List<TypeException> wrongFieldExceptions = n.fields.stream()
					.filter((field) -> -field.offset - 1 < superEntry.allMethods.size()) // attraverso il controllo
																							// sulla posizione (a
																							// sinistra), recupero solo
																							// i campi che fanno
																							// override
					.flatMap((field) -> { // eseguo il controllo sul tipo, se si manifestano errori, vengono
											// collezionati in modo ordinato
			...
			for (var exception : wrongFieldExceptions) {
				throw exception;
			} // se e solo se trovo eccezioni, le tiro

			List<TypeException> wrongTypeForMethodExceptions = n.methods.stream()
					.filter((method) -> method.offset < superEntry.allMethods.size()) // similmente alla gestione dei
																						// metodi, faccio un controllo
																						// sulla posizione
					.flatMap((method) -> { // eseguo il controllo sul tipo, se si manifestano errori, vengono
											// collezionati in modo ordinato
			...
			for (var exception : wrongTypeForMethodExceptions) {
				throw exception;
			} // se e solo se trovo eccezioni, le genero
		}
		return null;
	}
	 ...
	 * Molto banalmente, torniamo un tipo che vada bene per entrambi i possibili
	 * valori ritornati.
	 */
	@Override
	 ...
	 *
	 * Ho due superclassi, x e y. Una classe c le implementa entrambe
	 * (separatamente, quindi C implements X e C implements Y),
	 ...
	 *
	 * Il caso del sotto-tipo comune non lo consideriamo, perché non useremo
	...
	/**
	 * Possiamo assumere che i parametri siano corretti, perché verificato dal
	 * symbolTableVisitor
	 */
	...
	public TypeNode visitNode(ClassTypeNode n) throws TypeException {
		if (print)
			printNode(n);
		for (TypeNode f : n.allFields)
			visit(f);
		for (ArrowTypeNode m : n.allMethods)
			visit(m);
		return null;
	}
	...
	public TypeNode visitNode(NewNode n) throws TypeException {
		...
		// Controllo 1: verifico che il numero di argomenti corrisponda con i parametri
		// formali
		if (ct.allFields.size() != n.args.size()) {
			throw new TypeException("Wrong number of parameters in the invocation of " + n.id, n.getLine());
		}
		// Controllo 2: verifico, argomento per argomento, che il tipo del parametro sia
		// consistente
	...
	/**
	 * Devo recuperare l'entry type, deve essere un arrowTypeNode, se non lo è tiro
	 * l'eccezione
	 */
	@Override
	...
	/**
	 * Ho attaccato al node la stEntry, così ho tutte le informazioni sulla
	 * dichiarazione.
	 * Può essere un parametro o una variabile, per questo controllo che non sia una
	 * funzione.
	 * caso: x + 7 ma x può essere anche una funzione!
	 * Lo capisco entrando nella stEntry (si visita) e da lì capisco il tipo della
	 * dichiarazione.
	 */
	@Override
	...
	public TypeNode visitNode(ArrowTypeNode n) throws TypeException {
		...
		for (Node par : n.parlist)
			visit(par); // visito i parametri della funzione
