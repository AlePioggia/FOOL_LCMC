...
/**
 * Utilizziamo una symbol table, scegliamo come realizzazione una lista (ArrayList) di hashtable (HashMap),
 * per collegare usi a dichiarazioni.
 * Gli scope sono rappresentati dunque da tabelle che stanno dentro la lista.
 * Nella lista, avremo il fronte della lista che è lo scope attuale, in cui siamo posizionati, poi via via le tabelle
 ...
 *
 * L'obiettivo di questa visita sarà verificare la correttezza del sorgente, quindi dare i seguenti errori:
 ...
 *
 * L'effetto della visita è che l'AST si trasforma in un Enriched Abstract Syntax Tree (EAST),
 * dove ad alcuni nodi dell'AST sono attaccate STentry.
 * Le STentry sono palline attaccate agli usi degli identificatori.
 * E' stato possibile stabilire tale collegamento semantico grazie ai nomi degli identificatori,
 * che da ora in poi non verranno più usati.
 *
 * Si tratta di una visita, con i soliti settaggi, costruttore, metodi di visita, ecc..
 *
 * L'obiettivo della visita non è ritornare qualcosa! Bensì serve per arricchire l'albero.
 * L'arricchimento avviene solo quando è in grado di collegare un uso ad una dichiarazione.
 *
 * Ora dobbiamo occuparci di visitare l'EAST, stampando anche la STEntry. La stEntry deve essere visitata
 * allo stesso modo di node. Devo dunque generalizzare, includendo un'interfaccia Visitable, che vada bene
...
public class SymbolTableASTVisitor extends BaseASTVisitor<Void,VoidException> {
	...
	private int decOffset = -2; // counter for offset of local declarations at current nesting level.
								// starts with -2 due to our layout choice.
	 ...
	 * Quando esploro l'albero la tabella cresce quando entro negli scope, decresce quando ci esco.
	...
	public Void visitNode(FunNode n) {
		...
		//visito il corpo della funzione, prima ho elaborato tutte le dichiarazioni ed i parametri,
		// poi analizzo l'espressione, che usa le dichiarazioni (si ricorda il modello let/in)
		visit(n.exp);
		symTable.remove(nestingLevel--); //rimuovo la hashmap corrente poiche' esco dallo scope e decremento il nesting level
		decOffset=prevNLDecOffset; // restores counter for offset of declarations at previous nesting level 
		return null;
	}

	/**
	 * I VarNode aggiungono loro stessi alla lista, ovvero attacco i vari stentry alla mappa. Al nesting level corretto.
	 *
	...
	public Void visitNode(VarNode n) {
		if (print) printNode(n);
		//visito l'espressione (parte destra), prima di aggiungere la variabile nella symtable,
		// questo perché, se afferisce a variabili non dichiarate, devo subito lanciare l'eccezione
	...
	public Void visitNode(MethodNode n) throws VoidException {
		...
		// recupero dalla table l'id del metodo, se c'è già, vuol dire che è override, devo appurare che il tipo sia corretto
		...
		decOffset = -2;      //resetto il mio decOffset quando entro in un nuovo livello, parametri e variabili partono da -2 con il nuovo layout
		int parOffset = 1;   //parOffset è 1 per via del layout scelto, cresce per ogni parametro dichiarato
		...
		visit(n.exp); //visito il corpo della funzione, prima ho elaborato tutte le dichiarazioni ed i parametri,
					  // poi analizzo l'espressione, che usa le dichiarazioni (si ricorda il modello let/in)
		symTable.remove(nestingLevel--); //rimuovo la hashmap corrente poiche' esco dallo scope e decremento il nesting level
	...
	public Void visitNode(ClassNode node) {
		...
		if (node.superId != null && classTable.containsKey(node.superId)) {
			//recupero la stEntry della classe da qui eredita, in modo da recuperare i campi ed i metodi.
			STentry superClassEntry = symTable.get(0).get(node.superId);
			//imposto il classType con i field
			classType = new ClassTypeNode(new ArrayList<>(((ClassTypeNode) superClassEntry.type).allFields), new ArrayList<>(((ClassTypeNode) superClassEntry.type).allMethods));
		...
		//il class type varia in funzione dell'ereditarietà o meno, decOffset parte da -2 e decremento, parto dal livello globale!
		...
		onClassVisitScope = new HashSet<>(); //tiene traccia dei campi e metodi che visito nella classe attuale,
											 //mi consente di identificare doppie dichiarazioni (esulano dall'overriding)
		Map<String, STentry> virtualTable = new HashMap<>();
		var superClassVirtualTable = classTable.get(node.superId); //se non c'è ereditarietà rimane vuota
		if (node.superId != null) {
			//aggiorno la class table, copiando la virtual table dalla classe da cui si eredita (copio tutto e non solo il riferimento)
		...
		for (var field : node.fields) { //dichiarazione di campi
			...
			//recupero il campo dalla virtual table, se è presente, significa che è override
		...
		int previousNestingLevelDeclarationOffset = decOffset;	//salvo il valore del declarationOffset del nesting level precedente
		decOffset = 0;	//dopo avere salvato i vecchi offset setto quello attuale, perché i metodi vanno da 0 a n
		//se eredito l'offset non parte da 0, riparto da dove aveva lasciato la classe da cui eredito
		if (node.superId != null) {
	...
	public Void visitNode(ClassCallNode node) throws VoidException {
		if (print) printNode(node);
		//cerco la dichiarazione di id1 (che è una classe), attraverso la risalita della static chain of access link
		...
		if (entry == null) { //se non è dichiarata, è un errore!
			...
			//setto la stentry di id2, cercandola nella virtual table (raggiunta tramite class table) della classe del tipo RefTypeNode di id1
